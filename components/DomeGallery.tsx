...
export default function DomeGallery({
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver(entries => {
      ...
      // Responsive: scale down fit on narrow mobile screens so the full dome is visible
      const responsiveFit = w < 768 ? fit * 0.55 : fit;
      let radius = basis * responsiveFit;
      const heightGuard = w < 768 ? h * 0.95 : h * 1.35;
  ...
  // Auto-rotate horizontally at a slow speed
  useEffect(() => {
    const AUTO_ROTATE_SPEED = 0.02; // degrees per frame (~1.2 deg/sec at 60fps)

    const tick = () => {
      // Pause auto-rotation when dragging, inertia is active, or image is enlarged
      if (!draggingRef.current && !inertiaRAF.current && !focusedElRef.current) {
        const nextY = wrapAngleSigned(rotationRef.current.y + AUTO_ROTATE_SPEED);
